...
enum CacheKeyHasher {
    ...
    static func hexDigest(_ string: String, truncatedTo length: Int) -> String {
        let digest = sha256(Data(string.utf8))
    ...
    private static func sha256(_ data: Data) -> Data {
        ...
        for chunkStart in stride(from: 0, to: message.count, by: 64) {
            ...
            for i in 0..<16 {
                let byteIndex = chunk.startIndex + i * 4
                w[i] = UInt32(chunk[byteIndex]) << 24
                    | UInt32(chunk[byteIndex + 1]) << 16
                    | UInt32(chunk[byteIndex + 2]) << 8
                    | UInt32(chunk[byteIndex + 3])
            }
